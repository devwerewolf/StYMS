class Tilemap {
  constructor(img, tileWidth, tileHeight, tileReferences = []) {
    ...
    this.tileDictionary = Object.values(tileReferences).reduce((acc, cur) => {
      const {
        name
      } = cur;
      delete cur.name;
    ...
    if (visible) {
      for (let y = y1; y <= y2; y++) {
        for (let x = x1; x <= x2; x++) {
          ...
          dx++;
          this.collisionData[coordinates(dx - 1, dy)] = {
            stop
          };
        }
      ...
      this.collisionData[coordinates(dx, dy)] = { stop }
    }
  }
